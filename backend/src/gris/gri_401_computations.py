...
def _get_missing_fields(data: AIExtracted_GRI_401, substandard: str) -> list[str]:
    return [
        f for f in GRI_401_REQUIREMENTS[substandard]
...
def _compute_rate(numerator: Optional[int], denominator: Optional[int]) -> Optional[float]:
    ...
    return {
        ...
        "value": {
            "new_hires": {
                ...
                "rate": new_hire_rate,
                "by_gender": data.new_hires_by_gender.model_dump() if data.new_hires_by_gender else None,
                "by_age_group": data.new_hires_by_age_group.model_dump() if data.new_hires_by_age_group else None,
                "by_region": [r.model_dump() for r in data.new_hires_by_region] if data.new_hires_by_region else None,
            },
            "turnover": {
                "total": data.total_turnover,
                "rate": turnover_rate,
                "by_gender": data.turnover_by_gender.model_dump() if data.turnover_by_gender else None,
                "by_age_group": data.turnover_by_age_group.model_dump() if data.turnover_by_age_group else None,
                "by_region": [r.model_dump() for r in data.turnover_by_region] if data.turnover_by_region else None,
            }
        },
        "unit": "headcount",
        "metadata": {
            ...
            "has_gender_breakdown": data.new_hires_by_gender is not None or data.turnover_by_gender is not None,
            "has_age_breakdown": data.new_hires_by_age_group is not None or data.turnover_by_age_group is not None,
            "has_region_breakdown": data.new_hires_by_region is not None or data.turnover_by_region is not None,
        },
        "reason": None
    }
...
def compute_401_2_benefits(data: AIExtracted_GRI_401) -> Dict[str, Any]:
    ...
    full_time_only = [
        b.model_dump() for b in data.benefits
        if b.full_time and not b.part_time_or_temporary
    ]
    shared_benefits = [
        b.model_dump() for b in data.benefits
        if b.full_time and b.part_time_or_temporary
    ]
    not_provided = [
        b.model_dump() for b in data.benefits
        if not b.full_time
    ...
    return {
        ...
        "value": {
            ...
            "total_benefits_tracked": len(data.benefits)
        },
        "unit": "benefits",
        "metadata": {
            "base_year": data.base_year,
...
def compute_401_3_parental_leave(data: AIExtracted_GRI_401) -> Dict[str, Any]:
    ...
    computed_rates = []
    for entry in data.parental_leave_by_gender:
        return_rate = _compute_rate(entry.returned_to_work, entry.took_leave)
        retention_rate = _compute_rate(entry.still_employed_after_12_months, entry.returned_to_work)
